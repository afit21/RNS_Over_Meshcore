...
- **Real-world 2-3 hop test (~2km, real public repeater infrastructure, not a controlled rig):** the underlying MeshCore mesh itself is solid at this range — a standalone, RNS-independent MeshCore path-discovery test (`testscripts/path_discovery_diag.py`) succeeded 3/3, each in ~5s, and the official MeshCore app relayed a test message through 3 real repeater hops reliably. This interface's own RNSBIND/path-request broadcasts (single MeshCore fragment) also reached the far end reliably every time it was checked, and a 3-fragment ANNOUNCE did complete one full, clean round trip at this range (confirmed on both ends' logs, ~6s end-to-end). But most attempts at a full RNS round trip (`testscripts/relay_delivery_test.py`) did not succeed, including a retry with a 90s timeout (ruling out "test just gave up too early" as the whole explanation) — so multi-fragment CHANNEL-mode delivery over multiple hops is intermittent, not the repeaters/RF themselves (confirmed reliable via two independent, non-RNS methods above). Full details: [`fieldtests/reports/alpha-0.1-snapshot3.md`](fieldtests/reports/alpha-0.1-snapshot3.md).
